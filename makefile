...
E=.exe
CC=cl /nologo
CFLAGS=/DWIN32 /Zi /Fd$(BUILDDIR)^\ $(FLAGS)
LD=cl /nologo
LDFLAGS=/Zi /Fd$(BUILDDIR)^\ $(FLAGS)
...
$Bmu$O:	$Bmu.c;	$(CC) $(CFLAGS) -c -Isrc -Fo$@ $Bmu.c
...
$Bmu.c:	$Blburg$E $Bctomd$E src/mu.md src/_mu.c
	$Bctomd src/_mu.c src/mu.md
	$Blburg src/mu.md $@
...
$Bsizes$E:	$Bsizes$O;	$(LD) $(LDFLAGS) -Fe$@ $Bsizes$O

$Bctomd$E:	$Bctomd$O;	$(LD) $(LDFLAGS) -Fe$@ $Bctomd$O
...
$Bsizes$O:	src/sizes.c;		$(CC) $(CFLAGS) -c -Isrc -Fo$@ src/sizes.c

$Bctomd$O:	src/c_to_md.cpp;		$(CC) /Zi /EHsc /Fd$(BUILDDIR)\ $(FLAGS) -c -Isrc -Fo$@ src/c_to_md.cpp
...
mutest: all
	...
	$Brcc -target=mu -d -g9,// incr.i incr.mir 2> incr_d.txt
	$Brcc -target=mu -d -g9,// timm.i timm.mir 2> timm_d.txt